---
description: "이력서 진단 L0~L3 + 경로 분기. 사용: /leasume:assess <id>"
argument-hint: "<id>"
allowed-tools: Bash
---
...
`.leasume-session` 에 `assess_level` + 근거를 기록하고, 멘티에게 진단 결과와 다음 경로를 안내합니다.
...
`.leasume-session` 읽기. 파일이 없으면:
```
오류: {id} vault를 찾을 수 없습니다.
먼저 /leasume:init-resume-workspace {id} 를 실행하세요.
```
...
/leasume:interview-resume-hybrid {id} 를 실행하면 프로젝트 경험을 하나씩 채워나가게 됩니다.

다음: /leasume:interview-resume-hybrid {id}
```
