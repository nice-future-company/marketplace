---
description: "이력서 draft.md + final.html 생성. 사용: /leasume:export <id> [--html]"
...
`.leasume-session` 읽기. `pipeline_stage` 가 `init`, `assess`, `interview`, `format` 중 하나이면 export 미실행 상태이므로 중단:
```
아직 format-star-sbi가 완료되지 않았습니다.
/leasume:format-star-sbi {id} 를 먼저 실행하세요.
```
(`pipeline_stage` 허용 값: `export`. `init`/`assess`/`interview`/`format`이면 중단.)
...
추가 보완이 필요하면 /leasume:interview-resume-hybrid {id} 재실행 후 /leasume:format-star-sbi {id}, /leasume:export {id} 순으로 다시 실행하세요.
```
