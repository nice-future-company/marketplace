---
description: "파이프라인 진행 상황 표시. 사용: /leasume:status <id>"
argument-hint: "<id>"
allowed-tools: Bash
---
...
## 역할
`.leasume-session` 을 읽어 현재 파이프라인 진행 상태를 표시합니다.
...
`.leasume-session` 이 없으면:
```
{id} vault를 찾을 수 없습니다.
/leasume:init-resume-workspace {id} 를 먼저 실행하세요.
```
...
다음 명령: /leasume:{다음 pipeline_stage} {id}
```
...
- 순서: `init` → `assess` → `interview` → `format` → `export`
