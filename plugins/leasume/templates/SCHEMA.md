# leasume vault SCHEMA

이 파일은 `/leasume:init-resume-workspace <id>` 실행 시 멘티 vault에 자동 복사됩니다.

---

## vault 개요

이 디렉토리는 [leasume](https://github.com/leaphop/leaphop-marketplace/tree/main/plugins/leasume) 플러그인이 생성한 **개인 이력서 작업 vault**입니다.
...
`.leasume-session` 파일은 vault 루트에 위치하며 pipeline 상태를 추적합니다.

```yaml
---
mentee_id: "{id}"              # 멘티 식별자
vault_root: "/abs/path"        # 기록용 캐시, 경로 해소 진입점 아님
pipeline_stage: init|assess|interview|format|export
