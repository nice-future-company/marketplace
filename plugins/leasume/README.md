...
```bash
claude --plugin-dir /path/to/leaphop-marketplace/plugins/leasume
```
...
| 명령 | 역할 |
|------|------|
| `/leasume:init-resume-workspace <id>` | vault 생성 + 이력서 입력 + 데이터 처리 고지 |
| `/leasume:assess <id>` | 이력서 진단 (L0~L3) + 경로 분기 |
| `/leasume:interview-resume-hybrid <id>` | 인터뷰 + 5why 드릴다운 (핵심) |
| `/leasume:format-star-sbi <id>` | STAR/SBI 에피소드 구조화 |
| `/leasume:export <id> [--html]` | 이력서 draft.md + final.html 생성 |
| `/leasume:status <id>` | 진행 상황 확인 |
| `/leasume:help` | 사용법 안내 |
...
### L0 — 이력서 없음 또는 내용 부족
```
/leasume:init-resume-workspace <id>
/leasume:assess <id>
/leasume:interview-resume-hybrid <id>   ← 빈 템플릿에서 경험 채우기
/leasume:format-star-sbi <id>
/leasume:export <id>
```

### L1 — 최근 경력 미갱신
```
init-resume-workspace → assess → interview-resume-hybrid (최근 경험 우선) → format-star-sbi → export
```

### L2 — 보강 필요 (가장 많은 케이스)
```
init-resume-workspace → assess → interview-resume-hybrid → format-star-sbi → export
```
Why부재·정량없음·명사나열 항목을 drill-down으로 보강합니다.

### L3 — 완성형
```
init-resume-workspace → assess → interview-resume-hybrid (확인 위주) → format-star-sbi → export
```
...
| stage | 의미 |
|-------|------|
| `init` | init-resume-workspace 완료 |
| `assess` | assess 진행 준비 |
| `interview` | interview-resume-hybrid 진행 준비 |
| `format` | format-star-sbi 진행 준비 |
| `export` | export 완료 |
...
```
~/.leasume-vaults/<id>/         ← 기본 위치 (PII, 로컬 전용)
  raw/
...
```bash
# 환경변수
export LEASUME_VAULT_ROOT=/your/path

# 전역 설정 파일
echo '{"vault_root": "/your/path"}' > ~/.leasume-config.json
```
...
- `~/.leasume-vaults/<id>/` 삭제 시 모든 데이터 제거.
- `.leasume-session`, `.leasume-config.json`, `mentee-vault/` 는 `.gitignore` 로 git 추적 제외.

---

## Obsidian 연동 (선택)

vault 폴더(`~/.leasume-vaults/<id>/`)를 Obsidian에서 열면 wiki 링크와 그래프 뷰를 활용할 수 있습니다.
...
- **`LEASUME_VAULT_ROOT` 일관 유지**: 세션 간 환경변수가 바뀌면 vault를 찾지 못합니다.
...
LEASUME_VAULT_ROOT=../../etc ./scripts/resolve-vault-root.sh ok  # → ERROR: 절대경로 아님, exit 1
...
│   ├── init-resume-workspace.md
│   ├── assess.md
│   ├── interview-resume-hybrid.md
│   ├── format-star-sbi.md
│   ├── export.md
│   ├── status.md
│   └── help.md
├── skills/                          ← 공유 지시블록 (commands가 텍스트 참조)
│   ├── parse-resume-pdf/            ← PDF 파싱 규칙 (init-resume-workspace가 참조)
...
├── templates/
│   ├── SCHEMA.md                    ← vault 스키마 (init-resume-workspace가 복사)
│   └── .leasume-session.tmpl        ← 세션 파일 템플릿
├── tests/
...
- 7개 명령: init-resume-workspace, assess, interview-resume-hybrid, format-star-sbi, export, status, help
- 기존 스킬명 재사용 (행동기반 신규명 폐기): start→init-resume-workspace, dig→interview-resume-hybrid, frame→format-star-sbi
- 멘티 vault 구조 (william-vault 경량 fork)
- scripts/resolve-vault-root.sh — 경로 해소 단일 진입점 (절대경로·.. 검증 포함)
- templates/SCHEMA.md, .leasume-session.tmpl
- pipeline_stage enum: init|assess|interview|format|export
- L0~L3 진단 + 52세션 질문 프레임 인라인 (interview-resume-hybrid)
- STAR/SBI two-pass export (추정 마커 strip)
- scripts/validate.sh + tests/_fixture self-test
- 중복 스킬 제거: init-resume-workspace, interview-resume-hybrid, format-star-sbi (command로 대체)
- generate-korean-cover-letter: v1 미배선 표기 + disable-model-invocation: true

### v0.2.0
- skills-only 5단계 파이프라인 (init/parse/interview-resume-hybrid/star-sbi/cover-letter)
