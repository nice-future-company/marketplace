...
```
/resume:init jaewon-park-2026
    → sample-vault/raw/resume-original.md       (원본 이력서, 불변)
    → sample-vault/.resume-session              (pipeline_stage: init)
    → sample-vault/index.md
    → sample-vault/SCHEMA.md

/resume:assess jaewon-park-2026
    → sample-vault/wiki/projects/fintech-startup.md  (assess_flags 기록)
    → .resume-session pipeline_stage: interview, assess_level: L2

/resume:interview jaewon-park-2026
    ...
    → .resume-session pipeline_stage: star-sbi, dig_state 갱신

/resume:star-sbi jaewon-park-2026
    → sample-vault/wiki/episodes/redis-cache.md      (STAR 에피소드)
    → .resume-session pipeline_stage: export

/resume:export jaewon-park-2026
...
└── sample-vault/                           ← 전 파이프라인 후 vault 스냅샷 (가공)
    ...
    ├── index.md
    ├── SCHEMA.md
    └── .resume-session                    ← pipeline_stage: export (완주)
```
