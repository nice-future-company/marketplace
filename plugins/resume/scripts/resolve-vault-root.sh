...
# ── vault_root 해소 순서 ───────────────────────────────────────────────────
# 1) 환경변수 RESUME_VAULT_ROOT
# 2) ~/.resume-config.json 의 vault_root 필드
# 3) 기본값 ~/.resume-vaults

VAULT_ROOT=""

if [[ -n "${RESUME_VAULT_ROOT:-}" ]]; then
  VAULT_ROOT="$RESUME_VAULT_ROOT"
elif [[ -f "$HOME/.resume-config.json" ]]; then
  CONFIG_FILE="$HOME/.resume-config.json"
