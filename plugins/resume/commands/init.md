---
description: "멘티 vault 초기화 + 이력서 입력 + 데이터 처리 고지. 사용: /resume:init <id>"
argument-hint: "<id>"
allowed-tools: Bash
---
...
오류: id가 필요합니다.
사용법: /resume:init <id>
예시: /resume:init kim-frontend-2026
```
출력 후 중단.
...
{MENTEE_ROOT}/
  ...
  .resume-session   ← templates/.resume-session.tmpl 기반, id/vault_root/날짜 치환
```
...
`.resume-session` 초기값 (`templates/.resume-session.tmpl` 참조, 치환):
- `mentee_id`: `{id}`
...
- Y → `.resume-session` 의 `self_reported_staleness: true` 로 업데이트
- N → `self_reported_staleness: false` 유지

### 6단계: 완료 보고

`.resume-session` 의 `pipeline_stage` 를 `assess` 로 업데이트, `last_updated` 갱신.
