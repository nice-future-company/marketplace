...
`.resume-session` 의 `pipeline_stage` 가 `star-sbi` 또는 `export` 이면:
```
경고: 이미 인터뷰 완료된 vault입니다.
재진단 시 wiki/projects 페이지가 덮어쓰기될 수 있습니다.
...
괜찮아요. 빈 템플릿을 채우는 방식으로 interview를 진행합니다.
...
interview에서 가장 최근 경험부터 집중적으로 탐색합니다.
