---
description: "인터뷰 완료 항목을 STAR/SBI 에피소드로 구조화합니다. 사용: /resume:star-sbi <id>"
argument-hint: "<id>"
allowed-tools: Bash
---
...
## 역할
interview에서 수집한 프로젝트 내용을 STAR 또는 SBI 형식의 에피소드로 구조화합니다.
...
`.resume-session` 읽기. `pipeline_stage` 가 `init`, `assess`, `interview` 이면 포맷 미실행 상태이므로 중단:
```
아직 인터뷰가 완료되지 않았습니다.
/resume:interview {id} 를 먼저 실행하세요.
```
(`pipeline_stage` 허용 값: `star-sbi`, `export`. `init`/`assess`/`interview`이면 중단.)

`wiki/projects/` 파일 목록 확인. 없으면:
```
프로젝트 정보가 없습니다.
/resume:interview {id} 로 정보를 수집하세요.
```
...
다음: /resume:export {id}
또는 추정 항목을 먼저 보완하려면 /resume:interview {id} 재실행 후 export.
```
